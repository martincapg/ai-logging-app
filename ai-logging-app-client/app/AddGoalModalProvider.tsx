...
const AddGoalContext = createContext({ open: () => {}, close: () => {}, isOpen: false });

export function AddGoalModalProvider({ children }: { children: ReactNode }) {
  const [isOpen, setIsOpen] = useState(false);
  const open = () => setIsOpen(true);
  const close = () => setIsOpen(false);
  return (
    <AddGoalContext.Provider value={{ open, close, isOpen }}>
      {children}
      {isOpen && <AddGoalModal onClose={close} />}
    </AddGoalContext.Provider>
  );
}

export function useAddGoalModal() {
  return useContext(AddGoalContext);
}
